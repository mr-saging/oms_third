...
class PagesController extends CI_Controller {
    ...
    public function my_orders() {
        ...
        // fetch orders for this user and prepare view-friendly array
        $rawOrders = $this->PagesModel->get_orders_by_user($user_id);
        $orders = [];
        foreach ($rawOrders as $o) {
            $items = [];
            if (!empty($o->cart_json)) {
                $decoded = @json_decode($o->cart_json, true);
                if (is_array($decoded)) {
            ...
            $orders[] = [
                ...
                'created_at' => isset($o->order_date) ? date('M d, Y H:i', strtotime($o->order_date)) : '',
                'status' => $o->status,
    ...
    public function dashboard_admin() {
        ...
        $data['products']   = $this->PagesModel->getRecordsFood();
        // explicit total count (faster & consistent)
        $data['total_users'] = $this->PagesModel->countCustomers();

               // Build dynamic sales dataset: for every order row where status == Completed AND payment_status == Paid,
       // use order_date as label and total_amount as value.
        $sales_labels = [];
        $sales_values = [];
        if (!empty($data['orders']) && is_array($data['orders'])) {
            foreach ($data['orders'] as $o) {
                $status = strtolower(trim($o->status ?? ''));
                $pstatus = strtolower(trim($o->payment_status ?? ''));
                if ($status === 'completed' && $pstatus === 'paid') {
                    $label = isset($o->order_date) && strtotime($o->order_date) ? date('M d, Y H:i', strtotime($o->order_date)) : ('Order #' . ($o->orderID ?? ''));
                    $sales_labels[] = $label;
                    $sales_values[] = (float)($o->total_amount ?? 0);
                }
            }
        }
        $data['sales_labels'] = $sales_labels;
        $data['sales_values'] = $sales_values;

        // aggregated sales stats for chart ranges
        $sales_month = $this->PagesModel->get_sales_stats('monthly');
        $sales_week  = $this->PagesModel->get_sales_stats('weekly');
        $sales_day   = $this->PagesModel->get_sales_stats('daily');

        $data['sales_month_labels'] = $sales_month['labels'];
        $data['sales_month_values'] = $sales_month['values'];
        $data['sales_week_labels']  = $sales_week['labels'];
        $data['sales_week_values']  = $sales_week['values'];
        $data['sales_day_labels']   = $sales_day['labels'];
        $data['sales_day_values']   = $sales_day['values'];

        // --- NEW: dynamic user registration stats (monthly / weekly / daily) ---
        $users_month = $this->PagesModel->get_users_stats('monthly');
        $users_week  = $this->PagesModel->get_users_stats('weekly');
        $users_day   = $this->PagesModel->get_users_stats('daily');

        $data['users_month_labels'] = $users_month['labels'];
        $data['users_month_values'] = $users_month['values'];
        $data['users_week_labels']  = $users_week['labels'];
        $data['users_week_values']  = $users_week['values'];
        $data['users_day_labels']   = $users_day['labels'];
        $data['users_day_values']   = $users_day['values'];
    ...
    private function send_order_update_email($order, $customer, array $changes = []) {
        ...
        try {
            ...
            $mail->SMTPAuth   = true;
            $mail->Username   = 'INPUT_YOUR_EMAIL_HERE';
            $mail->Password   = 'INPUT_YOUR_EMAIL_PASSWORD_HERE';
            $mail->SMTPSecure = 'tls';
            $mail->Port       = 587;
            $mail->CharSet    = 'UTF-8';
            $mail->Encoding   = 'base64';

            $mail->setFrom('INPUTEMAIL', 'FoodsOUT');
            ...
            // Determine message type (compare lowercase)
            if ($status === 'completed' && $pstatus === 'paid') {
    ...
    {
        // only accept POST (AJAX or normal form)
        ...
        try {
            // Use PhpSpreadsheet to read file (CSV, XLSX, XLS supported)
            $spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load($tmp);
            $sheet = $spreadsheet->getActiveSheet();
            $rows = $sheet->toArray(null, true, true, true);

            if (count($rows) < 2) {
                $resp = ['success' => false, 'message' => 'File has no data rows'];
                return $this->output->set_content_type('application/json')->set_output(json_encode($resp));
            }

            // assume first row is header and columns are:
            // food_name, price, stocks, type, image, description
            $inserted = 0;
            $errors = [];
            ...
            for ($r = $start; $r <= count($rows); $r++) {
                $row = $rows[$r];
                // Map by header names if header exists (fallback to columns A..F)
                $food_name   = trim($row['A'] ?? '');
                $price       = is_numeric($row['B'] ?? '') ? floatval($row['B']) : 0;
                $stocks      = is_numeric($row['C'] ?? '') ? intval($row['C']) : 0;
                $type        = trim($row['D'] ?? '');
                $image       = trim($row['E'] ?? '');
                ...
                try {
                    $this->db->insert('food_details', $data);
            ...
            if (!empty($errors)) $msg .= ' Some rows failed.';

            $resp = ['success' => true, 'message' => $msg, 'errors' => $errors];
            return $this->output->set_content_type('application/json')->set_output(json_encode($resp));

        } catch (\Throwable $e) {
            $resp = ['success' => false, 'message' => 'Failed to parse file: '.$e->getMessage()];
            return $this->output->set_content_type('application/json')->set_output(json_encode($resp));
        }
    }
    ...
    public function insert_food() {
        ...
        $this->form_validation->set_rules('stocks', 'Stocks', 'required|integer|greater_than_equal_to[1]');
    ...
    private function send_delete_email($food_item) {
        ...
        try {
            ...
            $mail->SMTPAuth   = true;
            $mail->Username   = 'INPUT_YOUR_EMAIL_HERE';
            $mail->Password   = 'INPUT_YOUR_APP_PASSWORD_HERE';
            $mail->SMTPSecure = 'tls';
            $mail->Port       = 587;

            $mail->setFrom('INPUT_EMAIL', 'FoodsOUT Admin');
            $mail->addAddress('INPUT_RECIPIENT_EMAIL_HERE');
    ...
    private function send_receipt_email($order, $post = null) {
        ...
        try {
            ...
            $fromEmail = 'IPUT_YOUR_EMAIL_HERE';
     ...
     * Sets status = "Cancelled" and payment_status = "Failed", then emails the user.
     * Expected AJAX POST param: order_id
     */
    public function cancel_order()
    {
        ...
        $order = $this->PagesModel->get_order($order_id);
        if (! $order) {
            return $this->output->set_status_header(404)->set_output('Order not found');
        }

        // only owner can cancel
        if ((int)$order->userID !== $user_id) {
        ...
        if (! in_array($current, ['pending','processing'], true)) {
            return $this->output->set_status_header(409)->set_output('Order cannot be cancelled');
        }

        $oldStatus  = $order->status ?? '';
        $oldPayment = $order->payment_status ?? '';

        // perform update: set both status and payment_status as requested
        $updateData = [
            'status'         => 'Cancelled',
            'payment_status' => 'Failed'
        ];

        $updated = $this->PagesModel->updateOrder($order_id, $updateData);
        if (! $updated) {
            return $this->output->set_status_header(500)->set_output('Unable to cancel order');
        }

        // reload and notify user by email
        $order = $this->PagesModel->get_order($order_id);
        $customer = $this->PagesModel->get_customer($user_id);

        $changes = [
            'status' => ['old' => $oldStatus, 'new' => 'Cancelled'],
            'payment_status' => ['old' => $oldPayment, 'new' => 'Failed']
        ];

        if ($customer) {
            try {
                // re-use existing mailer — sends "Order Cancelled" notification
                $this->send_order_update_email($order, $customer, $changes);
            } catch (\Throwable $e) {
                log_message('error', 'Cancel notify email error: ' . $e->getMessage());
            }
        }
